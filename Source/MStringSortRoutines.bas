...
' FILENAME:     C:\CodeHelperFree\Source\MStringSortRoutines.bas
' AUTHOR:       Phil Fresle
' CREATED:      01-Dec-1999
...
Public Sub QuickSort2DStringsStart(ListArray() As String, _
    ...
    QuickSortStrings2D ListArray, lMin, lMax, lOrder, lCompareType, lIndex
End Sub

Private Sub QuickSortStrings2D(ListArray() As String, _
                               ByVal lLowerPoint As Long, _
                               ByVal lUpperPoint As Long, _
                               ByVal lOrder As Long, _
                               ByVal lCompareType As Long, _
    ...
    Do While lLowerPoint < lUpperPoint
        ...
        lMidPoint = QuickSortStringsPartition2D(ListArray, lLowerPoint, lUpperPoint, lOrder, lCompareType, lIndex)
        
        ' Recurively sort the smaller partition
        If (lMidPoint - lLowerPoint) <= (lUpperPoint - lMidPoint) Then
            QuickSortStrings2D ListArray, lLowerPoint, lMidPoint - 1, lOrder, lCompareType, lIndex
            lLowerPoint = lMidPoint + 1
        Else
            QuickSortStrings2D ListArray, lMidPoint + 1, lUpperPoint, lOrder, lCompareType, lIndex
            lUpperPoint = lMidPoint - 1
        End If
    Loop
End Sub

Private Function QuickSortStringsPartition2D(ListArray() As String, _
    ...
    Dim sTemp()     As String
    Dim lLowPoint   As Long
    Dim lHighPoint  As Long
    Dim lCount      As Long
    
    lLowPoint = LBound(ListArray, 1)
    lHighPoint = UBound(ListArray, 1)
    ReDim sTemp(lLowPoint To lHighPoint)
    ReDim sPivot(lLowPoint To lHighPoint)

    ' Select pivot point and exchange with first element
    lPivot = lLow + (lHigh - lLow) \ 2
    For lCount = lLowPoint To lHighPoint
        sPivot(lCount) = ListArray(lCount, lPivot)
    Next
    For lCount = lLowPoint To lHighPoint
        ListArray(lCount, lPivot) = ListArray(lCount, lLow)
    Next
    ...
    Do
        Do While lLowCount < lHighCount
            If StrComp(sPivot(lIndex), ListArray(lIndex, lLowCount), lCompareType) <> lOrder Then
                Exit Do
            Else
        ...
        Do While lHighCount >= lLowCount
            If StrComp(ListArray(lIndex, lHighCount), sPivot(lIndex), lCompareType) <> lOrder Then
                Exit Do
            Else
        ...
        ' Swap the items
        For lCount = lLowPoint To lHighPoint
            sTemp(lCount) = ListArray(lCount, lLowCount)
            ListArray(lCount, lLowCount) = ListArray(lCount, lHighCount)
            ListArray(lCount, lHighCount) = sTemp(lCount)
        Next
    ...
    For lCount = lLowPoint To lHighPoint
        ListArray(lCount, lLow) = ListArray(lCount, lHighCount)
        ListArray(lCount, lHighCount) = sPivot(lCount)
    Next
    
    QuickSortStringsPartition2D = lHighCount
End Function
